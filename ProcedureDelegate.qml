...
Item {
    ...
    signal updateClicked(procedureID: int, newFee: real, row: int);
    signal deleteClicked(procedureID: int, row: int);
    ...
    Rectangle {
        ...
        RowLayout {
            ...
            UFOButton {
                ...
                onClicked: {
                    root.deleteClicked(model.procedureID, model.row);
                }
            }
        ...
        Item {
            ...
            ColumnLayout {
                ...
                UFOButton {
                    ...
                    onClicked: {
                        if (!textFieldProcedureFee.acceptableInput) {
                            mainDialog.title = qsTr("ورودی ارائه شده به قسمت هزینه صحیح نیست.") + "\n" + qsTr("لطفاً قبل از ادامه هزینه صحیح را وارد کنید.");
                        ...
                        let newFee = parseFloat(textFieldProcedureFee.text);

                        root.updateClicked(model.procedureID, newFee, model.row);
                    }
                }
            }
        }
